...
def test_calc_gps_features_raises_if_gps_nans():
    ...
    with pytest.raises(
        RuntimeError, match="contains null values for lat_raw and lon_raw"
    ):
...
def test_calc_refine_features_handles_gps_gaps():
    ...
    df = read_test_sample("consecutive_gps_gaps.csv")
    df_gps = _calc_gps_features(
        df.filter(pl.col("lat_raw").is_not_null()).select(
            ["datetime", "lat_raw", "lon_raw"]
        )
    )
